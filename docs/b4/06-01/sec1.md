...
ユークリッドの互除法による $\rm{gcd}(a,b)$ の計算には $n$ 回の反復計算 (剰余演算) が必要であるならば、$a \ge f_{n+1}$ かつ $b \ge f_n$。
...
ユークリッドの互除法を用いて $\rm{gcd}(a,b)$ を求めるための反復回数は最大でも $\lfloor log_g(a) + 1 \rfloor$ 回である。
