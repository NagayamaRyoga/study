...
このアルゴリズムは、電子透かしを埋め込むための意味を保つコード変換とコード並べ替えに基づいている。
...
この技術はCoobergら (1998) を改良し、不透過述語を用いてメソッドが呼び出されないようにしている。
...
その結果、MylesとCollberg (2006) はSANDMARK (Collbergら 2003b) で2つの透かし手法を評価した。
...
カオス暗号化による電子透かしはKe-xinら (2009) によって定義された。
